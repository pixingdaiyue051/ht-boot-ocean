...
public class JedisUtil {
    ...
    private static class JedisUtilHolder {
        ...
        static {
            ...
            String profilesPath = "/hexk/github/hey-jude/deploy/assembly/application.yml";
            try (InputStream inputStream = new FileInputStream(profilesPath)) {
//                Yaml yaml = new Yaml();
...
//                INSTANCE.luaPath = fileMap.get("lua").toString();
                ...
                int timeout = Integer.parseInt(properties.getProperty("spring.redis.timeout"));
                String password = properties.getProperty("spring.redis.password");

                boolean blockWhenExhausted = Boolean.parseBoolean(properties.getProperty("spring.redis.block-when-exhausted"));
                boolean jmxEnabled = Boolean.parseBoolean(properties.getProperty("spring.redis.jmx-enabled"));
                int minIdle = Integer.parseInt(properties.getProperty("spring.redis.jedis.pool.min-idle"));
                int maxIdle = Integer.parseInt(properties.getProperty("spring.redis.jedis.pool.max-idle"));
                int maxTotal = Integer.parseInt(properties.getProperty("spring.redis.jedis.pool.max-active"));
                long maxWaitMillis = Long.parseLong(properties.getProperty("spring.redis.jedis.pool.max-wait"));
                ...
                INSTANCE.luaPath = properties.getProperty("file.lua");
                ...
                for (JedisLuaScriptEnum scriptEnum : JedisLuaScriptEnum.values()) {
                    ...
                    String script = INSTANCE.loadScriptFromDisk(luaFileName);
